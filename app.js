(function () {
  ...
  const I18N = {
    en: {
      ...
      stack_be: 'Backend', stack_fe: 'Frontend', stack_infra: 'Infra &amp; tooling',
    ...
    ru: {
      title: 'k1y0mi — Максим Чумаков · бэкенд- и фуллстек-разработчик',
      ...
      lede_about: 'Ядро — бэкенд: Python и Django. Последний год — фуллстек: фичи от базы данных до экрана. Помимо этого берусь за сложные измеримые задачи: ML-модели, оптимизацию токенов для LLM и системный софт.',
      ...
      stack_be: 'Бэкенд', stack_fe: 'Фронтенд', stack_infra: 'Инфра и тулинг',
  ...
  // language-aware numeric suffix (e.g. " yr" / " год")
  ...
  function applyLang(next) {
    ...
    // re-render already-finished count-ups with the new-language suffix
    document.querySelectorAll('[data-countup]').forEach(el => {
      if (el.dataset.done === '1') el.textContent = (el.dataset.prefix || '') + el.dataset.countup + sfx(el);
    });
